...
{
    //변수를 선언하는 방법은 타입과 변수명을을 선언하고 세미클론을 붙여준다.

    int test_int;       //정수를 할당할 때 int타입으로 변수를 선언한다.
    float test_float;   //정수도 가증하지만 소수의 값을 저장할 때 float타입의 변수를 선언한다.
    ...
    private void Start()
    {
        // int 1 = 0은 반복을 시작하는 숫자, i < 10은 i가 10 보다 작을 때까지 반복, i++는 i를 1씩 증가
       /* for(int i = 0; i < 10; i++)
        {
            Debug.Log(i);
        }*/
    ...
    /* void test_method(int a, int b)      //return을 안해주는 함수 함수명 염에 void로 선언해준다.
     {
         Debug.Log(a + b);
     }
